...
export function getCurrentLocation(onSucess, onFailure, googleMapsAPIKey) {
    ...
    debugLog("onFailure :: ", onFailure, typeof (onFailure), googleMapsAPIKey)
    ...
    Geolocation.getCurrentPosition(
        ...
        { enableHighAccuracy: false, timeout: 3000, maximumAge: 2000, distanceFilter: 200 }
    )
}

export function getAddress(latitude, longitude, onSuccess, onFailure, googleMapsAPIKey) {

    debugLog("onFailuregetAddress :: ", onFailure, typeof (onFailure), googleMapsAPIKey)
    if (googleMapsAPIKey == undefined || googleMapsAPIKey == null || googleMapsAPIKey.trim().length == 0) {
        showDialogue('Please configure Google Maps API Key!')
    ...
    Geocoder.from(latitude, longitude)
        .then(json => {
            ...
            if (json.results.length !== 0) {
                ...
                onFailure({ data: json, message: json.message || strings('generalNew.generalWebServiceError') })
            }
        })
        .catch(error => {
            debugLog('ERROR IN GETTING CURRENT LOCATION 125 :: ', error);
            onFailure({ data: error, message: error.message || strings('generalNew.generalWebServiceError') })
