...
import { saveUserDetailsInRedux, saveUserFCMInRedux, saveLanguageInRedux } from '../redux/actions/User'
...
class EditProfileContainer extends React.Component {
                ...
                <KeyboardAwareScrollView
                    ...
                    <View pointerEvents={this.state.isLoading ? 'none' : 'auto'} style={styles.mainViewStyle}>
                        ...
                        <View style={styles.textInputsContainer}>
                            ...
                            {/* Date Of Birth */}
                            { this.state.objProfileDetails.date_of_birth != undefined && this.state.objProfileDetails.date_of_birth != null && this.state.objProfileDetails.date_of_birth != "" ? 
                            <ProfileComponent
    ...
    _handleDatePicked = (date, identifier) => {
        ...
        this.state.objProfileDetails.date_of_birth = moment(datePicked, 'DD-MM-YYYY').format('YYYY-MM-DD')
     ...
     */
    buttonSavePressed = () => {
        if (this.validationsHelper.checkForEmpty(this.state.objProfileDetails.FirstName, strings('validationsNew.emptyFirstName')).length > 0) {
            showDialogue(strings('validationsNew.emptyFirstName'))
            return
        }
        if (this.validationsHelper.checkForEmpty(this.state.objProfileDetails.LastName, strings('validationsNew.emptyLastName')).length > 0) {
            showDialogue(strings('validationsNew.emptyLastName'))
            return
        }
        if (this.validationsHelper.checkForEmpty(this.state.objProfileDetails.date_of_birth, strings('validationsNew.emptyDateOfBirth')).length > 0) {
    ...
    onEditProfileSuccess = objSuccess => {
        ...
        showDialogue(objSuccess.message || strings('profileNew.profileUpdate'), '', [], () => {
            this.props.navigation.goBack()
        })
    }
