...
class CategoriesContainer extends React.Component {
            ...
            >
                ...
                <View
                    ...
                    <SafeAreaView style={styles.mainViewStyle}>
                        ...
                        {this.state.arrayCategories !== undefined && this.state.arrayCategories.length > 0
                            ? <FlatList
                            ...
                            : this.strOnScreenTitle.length > 0
                                ? <EDPlaceholderComponent onBrowseButtonHandler={this.buttonBrowsePressed} title={this.strOnScreenTitle} subTitle={this.strOnScreenSubtitle} />
                                : null}
                    </SafeAreaView>
                </View>
            </BaseContainer >
    ...
    renderCategoryItem = (categoryToLoad) => {
        return <ShopByItem onSelectionHandler={this.onCategorySelectionHandler} itemToLoad={categoryToLoad.item} />
    ...
    onGetCategoriesSuccess = objSuccess => {
        ...
        if (objSuccess.data.categories_list !== undefined && objSuccess.data.categories_list.length > 0) {

            let arrCategories = objSuccess.data.categories_list || []
            let totalCategoriesCount = objSuccess.data.categories_count || 0
            this.shouldLoadMore = this.state.arrayCategories.length + arrCategories.length < totalCategoriesCount
            this.setState({ arrayCategories: [...this.state.arrayCategories, ...arrCategories], isLoading: false });
        ...
        netStatus(isConnected => {
            if (isConnected) {
                ...
                let objGetCategoriesParams = {
                    ...
                    page_no: (this.state.arrayCategories && !isForRefresh) ? parseInt(this.state.arrayCategories.length / PAGE_SIZE_CATEGORIES) + 1 : 1,
                };
                if (!isForRefresh) {
                    this.setState({ isLoading: this.state.arrayCategories === undefined });
                }
                getCategories(objGetCategoriesParams, this.onGetCategoriesSuccess, this.onGetCategoriesFailure, this.props);
