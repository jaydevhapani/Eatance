...
import { getCMSPageDetails, updateTermsAndConditionsStatus } from '../utils/ServiceManager';
...
//#region NETWORK METHODS
  ...
  render() {
    ...
    return (
      ...
      <View
        ...
        <NavigationEvents onWillFocus={this.onWillFocusCMSContainer} onDidFocus={this.onDidFocusCMSContainer} />

        {/* THEME HEADER */}
        {/* {this.state.forSignUp !== true ?  */}
        <EDThemeHeader icon={ this.state.forSignUp !== true ? 'menu' : ''} onLeftButtonPress={ this.state.forSignUp !== true ? this.buttonMenuPressed : this.buttonBackPressed } title={this.title} /> 
        {/* : null } */}
          ...
          : this.state.objWebViewContent !== undefined
            ? <SafeAreaView style={{ flex: 1, backgroundColor: EDColors.white }}>
              ...
              <WebView
                ...
                style={[styles.webView, { flex: 1, paddingBottom: this.props.cartCount > 0 ? 80 : 0 }]}
                //hasIframe={true}
                scrollEnabled={true}
              />
              ...
              {this.cmsSlug == 'terms-and-conditions' && this.props.isLoggedIn
                ? <EDThemeButton
                  ...
                  style={{ marginBottom: 10, width: '30%', height: '8%', backgroundColor: EDColors.secondary, borderRadius: 5 }}
                  label={this.props.isTermsAndConditionsAccepted ? strings('buttonTitles.agreed') : strings('buttonTitles.agree')}
                  activeOpacity={this.props.isTermsAndConditionsAccepted ? 1 : 0.8}
                  onPress={this.props.isTermsAndConditionsAccepted ? undefined : this.buttonAgreePressed}
                />
                : null}
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (nextProps.navigation.state.params !== undefined &&
      ...
      this.props.navigation.state.params !== undefined &&
      this.props.navigation.state.params.routeParams !== undefined &&
      this.props.navigation.state.params.routeParams.cmsSlug !== nextProps.navigation.state.params.routeParams.cmsSlug
    ) {
      this.getCMSContent();
    }
    return true;
    ...
    netStatus(status => {
      if (status) {
        ...
        let paramsGetCMS = {
          ...
          cms_slug: this.props.navigation.state.params.routeParams.cmsSlug || ABOUT_US,
        };
