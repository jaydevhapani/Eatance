...
import { TextFieldTypes, debugLog, getProportionalFontSize, funGetDate } from '../utils/EDConstants';
...
class SignUpContainer extends React.PureComponent {
        ...
        return (
            <KeyboardAwareScrollView
                ...
                <View pointerEvents={this.state.isLoading ? 'none' : 'auto'} style={styles.mainViewStyle}>
                    ...
                    {/* CUSTOM HEADER */}
                    <EDThemeHeader onLeftButtonPress={this.buttonBackPressed} title={strings('signUpNew.title')} />
                    ...
                    <View style={styles.textFieldStyle}>
                        ...
                        <EDProfilePicture onImageSelectionHandler={this.onImageSelectionHandler} />
                        ...
                        />
                        <EDRTLView style={{ marginTop: 20, alignItems: "flex-start", marginHorizontal: 20 }}>
                            <Icon name={this.state.tncAccepted ? "check-box" : "check-box-outline-blank"} color={EDColors.primary} size={22} onPress={this.toggleTnc} />
                            <EDRTLView style={{ flexWrap: "wrap" , marginHorizontal: 5}}>
                                <EDRTLText title={strings("signUpNew.agree")} style={[styles.tncText]} />
                                <TouchableOpacity onPress={this.navigateToTnc}>
                                    <EDRTLText title={strings("signUpNew.tnc")} style={[styles.tncText, { borderBottomWidth: 1 }]}
                                    />
                                </TouchableOpacity>
                                <EDRTLText title={strings("signUpNew.and")} style={[styles.tncText]} />
                                <TouchableOpacity onPress={() => this.navigateToTnc(true)}>
                                    <EDRTLText
                                        title={strings("signUpNew.privacy")} style={[styles.tncText, { borderBottomWidth: 1 }]} />
                                </TouchableOpacity>
                            </EDRTLView>
                        </EDRTLView>
                        {/* SIGNUP BUTTON */}
                        ...
                        <EDRTLView style={styles.bottomContainer}>
                            ...
                            <EDRTLText style={styles.alreadyHaveText} title={strings('signUpNew.alreadyHave') + '? '} />
                            <EDUnderlineButton
    ...
    navigateToTnc = (forPrivacy) => {
        ...
        this.props.navigation.navigate("cmsFromSignUp", {
            routeParams: {
                screenName: forPrivacy == true ? strings("signUpNew.privacy") : strings("signUpNew.tnc"),
    ...
    state = {
        ...
        objRegistrationDetails: { firstName: '', lastName: '', email: '', password: '', mobile: '', date_of_birth: '' },
        tncAccepted: true,
    ...
    */
    ...
    _handleDatePicked = (date, identifier) => {
        ...
        this.state.objRegistrationDetails.date_of_birth = moment(datePicked, 'DD-MM-YYYY').format('YYYY-MM-DD')
        console.log('STORED DATE::::::::', moment(datePicked, 'DD-MM-YYYY').format('YYYY-MM-DD') )
        console.log('New DATE::::::::', this.state.objRegistrationDetails.date_of_birth )
    ...
    buttonSignUpPressed = () => {
        ...
        if (
            this.validationsHelper.checkForEmpty(this.state.objRegistrationDetails.firstName.trim(), strings('validationsNew.emptyFirstName')).length > 0 ||
            this.validationsHelper.checkForEmpty(this.state.objRegistrationDetails.lastName.trim(), strings('validationsNew.emptyLastName')).length > 0 ||
            this.validationsHelper.validateEmail(this.state.objRegistrationDetails.email.trim(), strings('validationsNew.emailEmpty')).length > 0 ||
            this.validationsHelper.validatePassword(this.state.objRegistrationDetails.password.trim(), strings('validationsNew.emptyPassword')).length > 0 ||
            this.validationsHelper.checkForMobileNumber(this.state.objRegistrationDetails.mobile.trim(), strings('validationsNew.emptyPhone')).length > 0 || 
            this.validationsHelper.checkForEmpty(this.state.objRegistrationDetails.date_of_birth.trim(), strings('validationsNew.emptyDateOfBirth')).length > 0
        ) {
            return;
        } else {
            if (this.state.tncAccepted)
                ...
                showDialogue(strings("signUpNew.tncError"))
        }
    }
    ...
    callSignUpAPI = () => {
        ...
        netStatus(isConnected => {
            if (isConnected) {
                ...
                signUpUser(objSignUpParams, this.onSignUpSuccess, this.onSignUpFailure, this.props);
            } else {
                showNoInternetAlert();
            }
        });
...
const styles = StyleSheet.create({
    ...
    mainViewStyle: { flex: 1, flexDirection: 'column', backgroundColor: EDColors.white, paddingBottom: 20 },
    textFieldStyle: { justifyContent: 'center', paddingBottom: 20 },
    signUpButton: { width: Metrics.screenWidth - 40, marginTop: 40 },
    bottomContainer: { justifyContent: 'center', marginTop: 15, alignItems: 'center' },
