...
class MyOrdersContainer extends React.Component {
    ...
    saveStorefromAsync = () => {
        if (this.props.objStoreDetails.store_id == undefined || this.props.objStoreDetails.store_id == undefined) {
            getSelectedStore(
                onSuccess => {
    ...
    onPressReviewButtonHandler = (orderToRepeat) => {
        this.setState({ isWriteAReviewVisible: true, deliveryFlag : orderToRepeat.delivery_flag.toLowerCase(), isReview : orderToRepeat.review === null ? true : false, store_id : orderToRepeat.store_obj.store_id, order_id : orderToRepeat.order_id, 
            driver_id : orderToRepeat.driver != undefined && orderToRepeat.driver != null ? orderToRepeat.driver.driver_id : '', store_name: orderToRepeat.store_name, store_image: orderToRepeat.store_obj.image,
            amount: orderToRepeat.total, driver_name: orderToRepeat.driver != undefined && orderToRepeat.driver != null ? orderToRepeat.driver.first_name : '', driver_image: orderToRepeat.driver != undefined && orderToRepeat.driver != null ? orderToRepeat.driver.image : '', 
    ...
    renderWriteAReviewDialogue = () => {
        ...
        return <EDPopupView
            ...
            <EDWriteSellerReview
                ...
                lan={this.props.lan}
                dismissWriteReviewDialogueHandler={this.dismissWriteReviewDialogueHandler}
                    ...
                    : <View
                        ...
                        <SafeAreaView style={styles.mainViewStyle}>
                            ...
                            <View style={styles.tabContainer}>
                                <SegmentedControlTab
                                    values={[strings('ordersNew.inProcess'), strings('ordersNew.past')]}
                            ...
                            <ScrollView scrollEnabled={false} ref={scrollView => this.scrollViewOrders = scrollView} bounces={false} pagingEnabled={true} horizontal={true}>
                                <View style={{ flexDirection: 'row' }}>
                                    {/* CURRENT ORDERS */}
                                    {this.arrayCurrentOrders !== undefined && this.arrayCurrentOrders.length > 0
                                        ? <EDOrdersFlatList lan={this.props.lan} onPressTrackButtonHandler={this.onPressTrackButtonHandler} refreshing={this.refreshing} onPullToRefreshHandler={this.onPullToRefreshHandler} cartCount={this.props.cartCount} isForCurrentOrder={true} currencySymbol={this.props.currencySymbol} style={{ width: Metrics.screenWidth }} arrayOrders={this.arrayCurrentOrders} />
                                        : (this.strCurrentOrderTitle || '').trim().length > 0
                                            ? <View style={{ width: Metrics.screenWidth }}>
                                                <EDPlaceholderComponent onBrowseButtonHandler={this.buttonBrowsePressed} title={this.strCurrentOrderTitle} subTitle={this.strCurrentOrderMessage} />
                                            </View>
                                            : null}

                                    {/* PAST ORDERS */}
                                    {this.arrayPastOrders !== undefined && this.arrayPastOrders.length > 0
                                        ? <EDOrdersFlatList onPressRepeatButtonHandler={this.onPressRepeatButtonHandler} refreshing={this.refreshing} onPullToRefreshHandler={this.onPullToRefreshHandler} cartCount={this.props.cartCount} currencySymbol={this.props.currencySymbol} style={{ width: Metrics.screenWidth }} arrayOrders={this.arrayPastOrders} 
                                            onPressReviewButtonHandler={this.onPressReviewButtonHandler}
                                        />
                                        : (this.strPastOrderTitle || '').trim().length > 0
                                            ? <View style={{ width: Metrics.screenWidth }}>
                                                <EDPlaceholderComponent onBrowseButtonHandler={this.buttonBrowsePressed} title={this.strPastOrderTitle} subTitle={this.strPastOrderMessage} />
                                            </View>
                                            : null}
                                </View>
                            </ScrollView>
    ...
    */
    onGetOrdersSuccess = objSuccess => {
        ...
        if (objSuccess.data.in_process !== undefined && objSuccess.data.in_process.in_process !== undefined && objSuccess.data.in_process.in_process.length > 0) {
        ...
        if (objSuccess.data.past !== undefined && objSuccess.data.past.past !== undefined && objSuccess.data.past.past.length > 0) {
    ...
    */
        ...
        netStatus(isConnected => {
            if (isConnected) {
                ...
                let objGetOrdersParams = {
                    ...
                    page_no: (this.state.arrayPastOrders && !isForRefresh) ? parseInt(this.state.arrayPastOrders.length / ORDERS_PAGE_SIZE) + 1 : 1,
                };
                // if (!isForRefresh) {
                //     this.setState({ isLoading: this.state.arrayPastOrders === undefined });
                // }
                this.setState({ selectedIndex: 0 })
                getOrders(objGetOrdersParams, this.onGetOrdersSuccess, this.onGetOrdersFailure, this.props);
            } else {
    ...
    clearCartAndNavigate = () => {
        ...
        saveSelectedStore(this.store_obj, () => { }, () => { });
    ...
    renderSwitchStoreConfirmationDialogue = () => {
        return (
            <EDPopupView isModalVisible={this.state.shouldShowConfirmationDialogue}>
                <EDConfirmationDialogue onYesClick={this.onYesPressed} onNoClick={this.onNoPressed} title={strings('storesList.switchStoreConfirmation')} />
            </EDPopupView>
        );
    }

    checkCurrentStore = (orderToRepeat) => {
        console.log("THIS.PROPS.OBJSTOREDETAILS::::::::::::", this.props.objStoreDetails)
    ...
    onPressRepeatButtonHandler = (orderToRepeat) => {
        if (orderToRepeat.timings.off === "open" && orderToRepeat.timings.closing === "open") {
            ...
            if (!arr.includes("1")) {
                ...
                let invalidItems = orderToRepeat.items.filter(data => { return data.isInStock == "0" })
                showDialogue(strings("generalNew.someitemsAvailable") + "\n\n" +
                    strings("generalNew.itemsunavailable") + "\n" +
                    invalidItems.map(data => data.name)
                    , "", [{
                        "text": strings("buttonTitles.proceed"), onPress: () => {
                            let newitems = orderToRepeat.items.filter(data => { return data.isInStock == "1" })
            ...
            showValidationAlert(strings("homeNew.notAcceptingOrdersMessage"))
    }
    ...
    repeatOrder = (orderToRepeat) => {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (items[i].is_customize === 1) {
                if (items[i].addons_category_list[0].addons_list[0].in_stock >= items[i].quantity) {
                    if (items[i].addons_category_list[0].addons_list[0].max_quantity >= items[i].quantity) {
                        items[i].quantity = parseInt(items[i].quantity)
                    }
                    else {
                        items[i].quantity = parseInt(items[i].addons_category_list[0].addons_list[0].max_quantity)
                        change_flag = true;
                        filteredArray.push(items[i].name + items[i].addons_category_list[0].addons_category);
                    }
                }
                else {
                    if (items[i].addons_category_list[0].addons_list[0].max_quantity <= items[i].addons_category_list[0].addons_list[0].in_stock) {
                        items[i].quantity = parseInt(items[i].addons_category_list[0].addons_list[0].max_quantity);
                        change_flag = true;
                        filteredArray.push(items[i].name + items[i].addons_category_list[0].addons_category);
                        ...
                        if (items[i].addons_category_list[0].addons_list[0].in_stock == 0 || items[i].addons_category_list[0].addons_list[0].in_stock == "0") {
                            removeArray.push(items[i].name + items[i].addons_category_list[0].addons_category);
                            change_flag = true
                        }
                        else {
                            items[i].quantity = parseInt(items[i].addons_category_list[0].addons_list[0].in_stock);
                            change_flag = true;
                            filteredArray.push(items[i].name + items[i].addons_category_list[0].addons_category);
                        }
                    }
        ...
        if (change_flag == true) {
            if (items.length !== 0) {
                showValidationAlert(strings('generalNew.itemAdjusted') + "\n\n" + filteredArray + "\n" + removeArray);
            }
        }
        if (removeArray.length !== 0) {
            tempArray = items.filter(data => { return !removeArray.includes(data.name + data.addons_category_list[0].addons_category) })
            items = tempArray
        }
        if (filteredArray.length !== 0) {
            tempArray = items.filter(data => { return !filteredArray.includes(data.name + data.addons_category_list[0].addons_category) })
            items = tempArray
        }
        if (items.length !== 0) {
        ...
        netStatus(isConnected => {
            if (isConnected) {
                if (orderDetails.driver !== undefined && orderDetails.driver !== null) {
                    this.props.navigation.navigate("trackOrder", { orderToTrack: orderDetails })
                } else {
