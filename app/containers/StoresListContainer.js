...
import {AppState, Linking, Platform, View, RefreshControl, StyleSheet, SafeAreaView, FlatList } from 'react-native'
...
import { debugLog, getProportionalFontSize, isRTLCheck, GOOGLE_API_KEY } from '../utils/EDConstants';
...
class StoresListContainer extends React.Component {
    ...
    constructor(props) {
        ...
        this.isForSwitchingStore = this.props.navigation.state !== undefined &&
    ...
    render() {
        return (
            <BaseContainer
                ...
                {/* SCREEN FOCUS EVENT */}
                <NavigationEvents onDidFocus={this.onDidFocusStoresListContainer} onWillFocus={this.onWillFocusStoresListContainer} />
                ...
                <View
                    ...
                    <SafeAreaView style={styles.mainViewStyle}>
                        ...
                        <SearchBar
                            ...
                            onSubmitEditing={() => { this.setState({ arrayStoresList: undefined }); this.callGetStoresListAPI() }}
                            inputContainerStyle={{
                                shadowOpacity: 0.25,
                                shadowRadius: 5,
                            ...
                            inputStyle={{ tintColor: EDColors.homeButtonColor, color: EDColors.text, fontFamily: EDFonts.regular, fontSize: getProportionalFontSize(14), textAlign: isRTLCheck() ? 'right' : 'left' }}
                            containerStyle={{ backgroundColor: EDColors.offWhite }}
                            style={{ tintColor: EDColors.homeButtonColor }}
                            onClear = {() => { this.setState({ arrayStoresList: undefined }); this.callGetStoresListAPI() }}
                        />
                        {/* CHECK IF WE HAVE NOTIFICATIONS, ELSE DISPLAY PLAHOLDER VIEW */}
                        {this.state.arrayStoresList !== undefined && this.state.arrayStoresList.length > 0
                            ? <FlatList
                            ...
                            : this.strOnScreenTitle.length > 0
                                ? <EDPlaceholderComponent title={this.strOnScreenTitle} subTitle={this.strOnScreenSubtitle} buttonTitle={this.buttonTitle} onBrowseButtonHandler={this.btnPress} />
                                : null}
                    </SafeAreaView>
                </View>
            </BaseContainer>
    ...
    onStoreSelectionHandler = (selectedStore) => {
        ...
        if (currentStore !== undefined && currentStore.store_id == selectedStore.store_id) {
            this.props.navigation.navigate(isRTLCheck() ? 'mainRTL' : 'main');
    ...
    renderStoreItem = (store) => {
        return <StoreInfoComponent onSelectionHandler={this.onStoreSelectionHandler} storeDetails={store.item} />
    }
    ...
    onConnectionChangeHandler = (isConnected) => {
        ...
        if (isConnected && this.state.arrayStoresList !== undefined && this.state.arrayStoresList.count == 0) {
            this.callGetStoresListAPI();
        }
    }
    //#endregion
    ...
    onGetStoresListSuccess = objSuccess => {
        ...
        if (objSuccess.data.store !== undefined && objSuccess.data.store.length > 0) {

            let arrStores = objSuccess.data.store || [];
            let totalStoresCount = objSuccess.data.stores_count || 0;
            this.shouldLoadMore = this.state.arrayStoresList.length + arrStores.length < totalStoresCount
            this.setState({ arrayStoresList: [...this.state.arrayStoresList, ...arrStores], isLoading: false });
            ...
            if (isConnected) {
                ...
                if (this.props.currentLocation !== undefined && this.props.currentLocation !== null && this.props.currentLocation.latitude !== undefined) {
                    let objGetStoresParama = {
                        ...
                        page_no: (this.state.arrayStoresList && !isForRefresh) ? parseInt(this.state.arrayStoresList.length / PAGE_SIZE_STORES_LIST) + 1 : 1,
                        sort_by: 'distance',
                    ...
                    if (!isForRefresh) {
                        this.setState({ isLoading: this.state.arrayStoresList === undefined, isMoreLoading: this.state.arrayStoresList !== undefined && this.state.arrayStoresList.length !== 0 ? true : false });
                    }
                    getStores(objGetStoresParama, this.onGetStoresListSuccess, this.onGetStoresListFailure, this.props);
                }
                else {
                    this.setState({ isLoading: true });
                    var paramPermission =
                    ...
                    checkLocationPermission(
                        ...
                        onFailure => {
                            ...
                            if (onFailure == RESULTS.BLOCKED) {
                                ...
                                this.strOnScreenTitle = strings('generalNew.locationPermission');
                            }
                            else {
