...
import { debugLog, isRTLCheck, getProportionalFontSize, funGetFrench_Curr, STORE_ERROR } from '../utils/EDConstants';
...
class CheckOutContainer extends React.PureComponent {
    ...
    deleteModalOpen = () => {
        return (
            <EDPopupView isModalVisible={this.state.isDeleteVisible}>
                <EDConfirmationDialogue onYesClick={this.onYesChangeHandler} onNoClick={this.onNoClick} title={strings('checkoutNew.deleteMsg')} />
            </EDPopupView>
        );
    };
                    ...
                    <View>
                        {item.value !== undefined && item.value !== null && item.value !== "" ?
                            <EDRTLView style={style.container}>
                                <EDRTLText style={style.itemTitle} title={item.label !== undefined ? item.label : ""} />
                                <EDRTLText
                                    style={[style.price,
                                    {
                                        color: item.label_key.toLowerCase() == "discount"
                                            ? EDColors.text
                                            : EDColors.text
                                    }]}
                                    title={(item.label_key.toLowerCase() === "discount"
                                        ? ' - '
                                        : item.label_key.toLowerCase().includes('delivery')
                                            ? ' + ' : item.label_key.toLowerCase().includes('service') ? ' + ' : '')
                                        + this.props.currencySymbol + funGetFrench_Curr(item.value, 1, this.props.lan)}
                                />
                            </EDRTLView> : null}
                    </View> :
                    <EDRTLView style={style.container}>
                        <EDRTLText style={style.itemTitleBold} title={item.label !== undefined ? item.label : ""} />
                        <EDRTLText style={style.priceBold} title={this.props.currencySymbol + funGetFrench_Curr(item.value, 1, this.props.lan)} />
                    </EDRTLView>
                }
            </View>
            ...
            >
                ...
                <View
                    ...
                    {this.featured_items !== undefined && this.featured_items !== null && this.featured_items.length !== 0 ?
                        <View>
                          <EDRTLText title={strings("checkoutNew.alsoOrdered")} style={[style.priceBold, { marginTop: 10, marginHorizontal: 10, marginBottom: 5 }]} />
                          {/* <Text>Teest</Text> */}
                          <FlatList
                            style={{ marginVertical: 5, marginBottom: 10, marginHorizontal: 10 }}
                    ...
                    {this.priceArray.length > 0 ? (
                        <View>
                            ...
                            <EDRTLView style={{ marginVertical: 10, paddingHorizontal: 10, backgroundColor: EDColors.white }}>
                                <TouchableOpacity style={{ flex: 1, paddingVertical: 15, flexDirection: isRTLCheck() ? 'row-reverse' : 'row' }} onPress={this.couponHandler}>
                                    <EDRTLText style={[style.couponText, { color: this.addToCartDict.is_apply ? EDColors.primary : EDColors.text }]} title={this.addToCartDict.is_apply ? this.coupon_name : strings('checkoutNew.promo')} />
                                    <MaterialIcon size={20} color={EDColors.primary}
                                        name={this.addToCartDict.is_apply ? 'close' : 'keyboard-arrow-right'} />
                            ...
                            {console.log("CURRENCY CHECK", this.addToCartDict.total, this.props.lan, funGetFrench_Curr(18000, 1, "en"))}
                            <CheckOutBottomComponent
                                pointerEvents={this.state.isLoading ? 'none' : 'auto'}
                                title={
                                    this.addToCartDict != undefined
                                        ? this.props.currencySymbol + funGetFrench_Curr(this.addToCartDict.total, 1, this.props.lan)
                                        : ''
                                }
    ...
    renderFeaturedItems = (data) => {
        return (
            (data.item.item_in_stock) != 0 ? 
            <View style={[ style.featuredView, {flexDirection: 'column'}]} >
                <TouchableOpacity onPress={() => this.onFeaturedPress(data.item)}
                  style={{ width: Metrics.screenWidth * .33, paddingEnd: (Metrics.screenWidth * .30 - Metrics.screenWidth * .28) / 2 }}
                >
                    <EDImage source={data.item.image} style={{ width: Metrics.screenWidth * .33, height: 70, borderTopLeftRadius: 6, borderTopRightRadius: 6, resizeMode: 'contain' }} />
                    <EDRTLText title={data.item.name} style={{ fontSize: getProportionalFontSize(14), fontFamily: EDFonts.bold, marginHorizontal: 5, marginTop: 5, color: EDColors.black }} numberOfLines={1} />
                    <EDRTLText style={[style.price1, { textDecorationLine: data.item.offer_price !== "" ? "line-through" : null }]} title={data.item.price !== null && data.item.price !== "0" && data.item.price !== "0.00" ? this.props.currencySymbol + data.item.price : ""} />
                    {data.item.offer_price !== "" ?
                        <EDRTLText style={[style.price1, { color: EDColors.primary, marginTop: 0 }]} title={this.props.currencySymbol + data.item.offer_price} /> : null}
                </TouchableOpacity>
                <TouchableOpacity onPress={() => this.onPressAddtoCartItemHandler(data.item, 1)} >
                  <EDRTLText title={strings("productDetailsNew.addToCart")} style={style.addBtn}  />
                </TouchableOpacity>
          </View>
        : <View />
        )
      ...
      onPressAddtoCartItemHandler = (item, qty) => {
        ...
        if (item.is_customize === "0") {
            ...
            if (this.addToCartDict.items !== undefined && this.addToCartDict.items.length > 0) {
        ...
        if (
            ...
            if (cartdata.items.length > 0) {
                ...
                var repeatArray = arrCartItems.filter(item => {
                    return item.menu_id == Category_data.menu_id && item.is_customize == 0;
                });
        ...
        return (
            ...
            <EDPopupView isModalVisible={this.state.visible}>
                <EDItemDetails
                    ...
                    isOpen={true}
                    cartData={this.addToCartDict !== undefined && this.addToCartDict.items.length !== 0 ? this.addToCartDict.items : []}
    ...
    onAddtoCartSuccess = objSuccess => {
        ...
        this.priceArray =
            this.addToCartDict.price != undefined &&
                ...
                : [];
        if(this.CartItemsArray.inactive_items != null && this.CartItemsArray.inactive_items != undefined && this.addToCartDict.items.length > 0){
            var inactiveItems = this.CartItemsArray.inactive_items.filter(data => { return data.menu_id != "0" })
            showDialogue(strings("generalNew.itemAdjusted") + "\n\n" + inactiveItems.map(data => data.name))
        }
    ...
    updateCart(response) {
        ...
        console.log('MENU_SUGGESTION :::::::: ', JSON.stringify(response.menu_suggestion));
        ...
        this.setState({
            isLoading: false, key: this.state.key + 1
        });
    }
    ...
    navigateToPayment = () => {
        ...
        if (this.addToCartDict.chk_in_stock !== undefined && this.addToCartDict.chk_in_stock !== null)
        ...
        if (this.itemsNameArray.length > 0) {
            ...
            this.alertString = this.itemsNameArray.length > 1 ?
            ...
            let normalItems = stock_error_items.filter(key => { return key.is_customize == 0 })
            let nameArray = normalItems.map(key => key.name + " :\n" + strings('checkoutNew.currentStock') + " - " + key.in_stock + " , " + strings('checkoutNew.maxQuantity') + " - " + key.max_quantity).join('\n\n')
            let customItems = stock_error_items.filter(key => { return key.is_customize !== 0 })
            let nameArrayCustom = []
            customItems.map(data => {
              data.addons_category_list.map(
                item => {
                  item.addons_list.map(
                    items => {
                      debugLog("TEST ADDON :::::", data.name, item.addons_category, items.in_stock)
                      let temp = {
                        name: data.name,
            ...
            let cutomNameArray = nameArrayCustom.map(key => key.name + " (" + key.addons_category + ")" + " :\n" + strings('checkoutNew.currentStock') + " - " + key.in_stock + " , " + strings('checkoutNew.maxQuantity') + " - " + key.max_quantity).join('\n\n')
            debugLog("NAME ARRAY ::::", nameArray, nameArrayCustom)
            let alertMsg = strings("checkoutNew.noStock") + nameArray + "\n\n" + cutomNameArray
  ...
  addOrderData = (checkoutDetail) => {
    ...
    netStatus(isConnected => {
      if (isConnected) {
        ...
        objaddOrderParams.order_date = Moment(new Date()).format("DD-MM-YYYY hh:mm A");
...
export const style = StyleSheet.create({
    ...
    featuredView:{
        width: Metrics.screenWidth * .33, marginEnd: (Metrics.screenWidth * .30 - Metrics.screenWidth * .26) / 2,
                    backgroundColor: EDColors.white,
                    borderRadius: 6,
                    elevation: 1,
                    shadowColor: "#000",
    ...
    lastBtn: {
        marginHorizontal: 20, height: 40, borderRadius: 20, paddingHorizontal: 40
    },
    couponText: {
        flex: 1,
