...
import { debugLog, ProductsListType, isRTLCheck, getProportionalFontSize } from '../utils/EDConstants';
...
class ProductsListContainer extends React.Component {
    ...
    constructor(props) {
        ...
        this.listType = this.props.navigation.state.params !== undefined && this.props.navigation.state.params.listType !== undefined
            ? this.props.navigation.state.params.listType
            : ProductsListType.other;
        this.entityFromHomeScreen = this.props.navigation.state.params !== undefined && this.props.navigation.state.params.entityFromHomeScreen !== undefined
    ...
    getScreenTitle = () => {
        if (this.listType == ProductsListType.category) {
            return this.entityFromHomeScreen !== undefined ? this.entityFromHomeScreen.name || '' : ''
        ...
        return (
            ...
            >
                ...
                <SearchBar
                    ...
                    inputStyle={{ color: EDColors.text, fontFamily: EDFonts.regular, fontSize: getProportionalFontSize(14), textAlign: isRTLCheck() ? 'right' : 'left' }}
                />
                    ...
                    <SafeAreaView style={styles.mainViewStyle}>
                        ...
                        {this.state.arrayProducts !== undefined && this.state.arrayProducts.length > 0
                            ...
                            : this.strOnScreenMessage.length > 0
                                ? <EDPlaceholderComponent buttonTitle={strings('buttonTitles.okay')} onBrowseButtonHandler={this.buttonBrowsePressed} title={this.strOnScreenMessage} subTitle={this.strOnScreenSubtitle} />
                                : null}
                    </SafeAreaView>
                </View>
            </BaseContainer >
    ...
    renderProductItem = (productToLoad) => {
        return <ProductComponent
            ...
            isOpen={((this.props.objStoreDetails.timings || {}).closing || '').toLowerCase() == 'open' ? true : false}
    ...
    buttonAddSingleItemPressed = (productToAdd) => {
        if (this.props.cartDetail !== null && this.props.cartDetail.items !== undefined && this.props.cartDetail.length !== 0) {
            let cartData = this.props.cartDetail.items
            let arr = cartData.filter(p => { return p.menu_id == productToAdd.menu_id })
            if (arr.length !== 0) {
                if (productToAdd.item_in_stock !== null && productToAdd.item_in_stock !== 0 && productToAdd.item_in_stock > arr[0].quantity) {
                    if (productToAdd.item_max_quantity > arr[0].quantity)
                        this.storeData(productToAdd)
                    else
                        showValidationAlert(strings("generalNew.maxQuantity") + productToAdd.item_max_quantity)
                }
                else
                    showValidationAlert(strings("generalNew.noMoreStock"))
            }
            else {
                if (productToAdd.item_in_stock !== null && productToAdd.item_in_stock !== 0)
                    this.storeData(productToAdd)
                else
                    showValidationAlert(strings("generalNew.noMoreStock"))
            }
    ...
    onGetProductsSuccess = objSuccess => {
        ...
        if (objSuccess.data.product_list !== undefined && objSuccess.data.product_list instanceof Array && objSuccess.data.product_list.length > 0) {
            ...
            this.shouldLoadMore = this.state.arrayProducts.length + arrProducts.length < totalProductsCount
            this.setState({ arrayProducts: [...this.state.arrayProducts, ...arrProducts], isLoading: false });
    ...
    */
    callProductsAPI = (isForRefresh = false) => {
        ...
        if (this.state.arrayProducts !== undefined && this.state.arrayProducts.length > 0 && !this.shouldLoadMore) {
            return;
        }
        ...
        var strSelectedCategoryIDs = this.props.objFilter.arraySelectedCategoryIDs.join();
