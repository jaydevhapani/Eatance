...
class NotificationsContainer extends React.Component {
    ...
    render() {
        return (
            ...
            >
                {/* SCREEN FOCUS EVENT */}
                <NavigationEvents onWillFocus={this.onWillFocusNotificationsContainer} onDidFocus={this.onDidFocusNotificationsContainer} />
                ...
                <View
                    ...
                    <SafeAreaView style={styles.mainViewStyle}>
                        ...
                        {this.state.arrayNotifications !== undefined && this.state.arrayNotifications.length > 0
                            ? <FlatList
    ...
    onGetNotificationsSuccess = objSuccess => {
        ...
        if (objSuccess.data.notification !== undefined && objSuccess.data.notification.length > 0) {

            let arrNotifications = objSuccess.data.notification || []
            let totalNotificationsCount = objSuccess.data.notificaion_count || 0
            this.shouldLoadMore = this.state.arrayNotifications.length + arrNotifications.length < totalNotificationsCount
            this.setState({ arrayNotifications: [...this.state.arrayNotifications, ...arrNotifications], isLoading: false });
        ...
        netStatus(isConnected => {
            if (isConnected) {
                ...
                let objGetNotificationParams = {
                    ...
                    count: PAGE_SIZE,
                    page_no: (this.state.arrayNotifications && !isForRefresh) ? parseInt(this.state.arrayNotifications.length / PAGE_SIZE) + 1 : 1,
                };
                if (!isForRefresh) {
                    this.setState({ isLoading: this.state.arrayNotifications === undefined });
                }
                getNotifications(objGetNotificationParams, this.onGetNotificationsSuccess, this.onGetNotificationsFailure, this.props);
