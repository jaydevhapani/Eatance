...
import { isRTLCheck, debugLog, getProportionalFontSize, funGetFrench_Curr } from '../utils/EDConstants';
        ...
        return (
            <View style={style.prescriptionView}>
                {this.state.prescriptionImageSource !== undefined
                    ? <Image
                        style={[style.prescriptionImage,
                        {
                            marginTop: 0
                        }]}
                        source={{ uri: this.state.prescriptionImageSource !== undefined && this.state.prescriptionImageSource.uri !== undefined ? this.state.prescriptionImageSource.uri : "" }}
                    />
                    : null}
                ...
                {this.state.prescriptionImageSource == undefined
                    ? <TouchableOpacity
                        style={{
                            ...
                            marginTop: this.state.prescriptionImageSource !== undefined && this.state.prescriptionImageSource.uri !== undefined ? 20 : 0
                        }}
                        onPress={this.buttonAddPrescriptionPressed}
                    >
                        <MaterialIcon size={20} color={EDColors.homeButtonColor} name={'note-add'} />
                        <EDRTLText style={style.addPrescriptionText} title={
                            this.state.prescriptionImageSource !== undefined && this.state.prescriptionImageSource.uri !== undefined ?
                                strings("payment.updatePrescription") : strings("payment.addPrescription")} />
                    </TouchableOpacity>
                    : null}
            </View >
        )
                ...
                <View
                    ...
                    <View
                        ...
                        <RadioGroup
                            ...
                            style={[style.radioGroupStyle, { flexDirection: isRTLCheck() ? 'row-reverse' : 'row' }]}
                    ...
                    <CheckOutBottomComponent
                        ...
                        title={
                            this.props.navigation.state.params.checkoutDetail !== undefined
                                ? this.props.currencySymbol + funGetFrench_Curr(this.props.navigation.state.params.checkoutDetail.total, 1, this.props.lan)
                                : ''
                        }
    ...
    addOrderData = () => {
        netStatus(isConnected => {
            if (isConnected) {
                ...
                var objaddOrderParams = this.props.navigation.state.params
                    .checkoutDetail;
...
export const style = StyleSheet.create({
    ...
    txtStyle: {
        fontFamily: EDFonts.regular, fontSize: getProportionalFontSize(14),
        color: EDColors.textAccount
    },
    paymentSelectTxt: {
        marginTop: 10, padding: 15, backgroundColor: EDColors.white, fontFamily: EDFonts.bold, color: EDColors.primary, fontSize: getProportionalFontSize(20)
    },
    radioGroupStyle: {
        marginHorizontal: 10, backgroundColor: EDColors.white, marginTop: 10
    },
    prescriptionText: {
        // flex:1,
        marginHorizontal: 15,
    ...
    prescriptionImage: {
        width: "100%", height: Metrics.screenWidth * 0.45,
        borderRadius: 5,
        overflow: 'hidden'
    },
    addPrescriptionImage: {
        width: 20, height: 20, tintColor: EDColors.primary, marginHorizontal: 10
    },
    addPrescriptionText: {
