...
import { flushAllData, saveLanguage, saveSelectedStore, setIsTermsAndConditionsAccepted } from '../utils/AsyncStorageHelper';
import { saveUserDetailsInRedux, saveLanguageInRedux, saveTermsAcceptedStatus } from '../redux/actions/User';
...
const APP_STORE_LINK = 'http://itunes.apple.com/app/id1519383164';
const PLAY_STORE_LINK = 'https://play.google.com/store/apps/details?id=com.eatanceapp.liquor';
...
class SideBar extends React.PureComponent {
  ...
  /** MAIN RENDER METHOD */
  render() {
    let arrCMSPages = ((this.props.arrayCMSPages)).map(itemToIterate => { return { isAsset: true, route: 'cms', screenName: itemToIterate.name, icon: { uri: itemToIterate.cms_icon }, cmsSlug: itemToIterate.cms_slug }; });
    let arrTemp = this.setupSideMenuData();
    let arraySideMenuData = arrTemp.concat(arrCMSPages);

    this.arrayFinalSideMenu =
      this.props.isLoggedIn
        ? arraySideMenuData.concat({ isAsset: true, route: 'SignOut', screenName: strings('sidebarNew.signout'), icon: Assets.logout })
        : arraySideMenuData;
  ...
  setupSideMenuData = () => {
    return [
      ...
      { route: 'account', screenName: strings('sidebarNew.account'), icon: 'settings' },
      { route: 'myOrders', screenName: strings('sidebarNew.myOrder'), icon: 'receipt' },
      { route: 'notifications', screenName: strings('sidebarNew.notification'), icon: 'notifications-active' },
  ...
  logoutDialogue = () => {
    return (
      <EDPopupView isLogoutLoading={this.state.isLoading} isLoading={this.state.isLoading} isModalVisible={this.state.isLogout}>
        <EDConfirmationDialogue isLoading={this.state.isLoading} onYesClick={this.onYesClick} onNoClick={this.onNoClick} title={strings('sidebarNew.logoutConfirm')} />
      </EDPopupView>
    );
  }
  //#endregion
  ...
  onPressHandler = (_selectedItem, selectedIndex) => {
    ...
    // CLOSE DRAWER
    if (this.arrayFinalSideMenu[selectedIndex].screenName !== strings('sidebarNew.signout')) {
    ...
    // NOTIFICATION
    else if (this.arrayFinalSideMenu[selectedIndex].screenName === strings('sidebarNew.notification')) {
    ...
    // ORDERS
    else if (this.arrayFinalSideMenu[selectedIndex].screenName === strings('sidebarNew.myOrder')) {
      if (this.props.isLoggedIn) {
        // SAVE SELECTED ITEM IN REDUX
    ...
    // ACCOUNT
    else if (this.arrayFinalSideMenu[selectedIndex].screenName === strings('sidebarNew.account')) {
      // if (this.props.isLoggedIn) {
      // SAVE SELECTED ITEM IN REDUX
    ...
    // RATE APP
    else if (this.arrayFinalSideMenu[selectedIndex].screenName === strings('sidebarNew.rate')) {
      this.openStore();
    }
    // SHARE APP
    else if (this.arrayFinalSideMenu[selectedIndex].screenName === strings('sidebarNew.share')) {
      this.shareApp();
    }
  ...
  onLogoutSuccess = (_objSuccess) => {
    ...
    flushAllData(
      _response => {
        ...
        saveSelectedStore(undefined, onSuccess => { }, onFailure => { });
...
const style = StyleSheet.create({
  ...
  navHeader: { flex: 1, flexDirection: 'column-reverse', backgroundColor: EDColors.primary, alignItems: 'center', justifyContent: 'center' },
