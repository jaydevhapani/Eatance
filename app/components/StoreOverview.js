...
export default class StoreOverview extends React.PureComponent {
  ...
  render() {
    return (
      this.props.storeDetails !== undefined && this.props.storeDetails.address !== undefined && this.props.storeDetails.address.trim().length > 0
        ? <View
          ...
          <EDRTLView>
            ...
            {/* CALL BUTTON */}
            {this.props.storeDetails.phone_number !== undefined && this.props.storeDetails.phone_number.trim().length > 0
              ? <Icon reverse raised containerStyle={{ marginHorizontal: 8, marginVertical: 0 }} size={13}
                name={'call'} color={EDColors.homeButtonColor} onPress={this.buttonCallPressed} />
              : null}

            {/* DIRECTIONS BUTTON */}
            {this.props.storeDetails.latitude !== undefined && this.props.storeDetails.longitude !== undefined
              ? <Icon reverse raised size={13} containerStyle={{ margin: 0 }} name={'directions'} color={EDColors.homeButtonColor} onPress={this.buttonDirectionsPressed} />
              : null}

          </EDRTLView>

          <View style={{ backgroundColor: EDColors.separatorColor, margin: 10, marginHorizontal: 10, height: 1 }} />

          {/* BOTTOM CONTAINER */}
          <EDRTLView style={style.bottomContainer}>
            {/* STAR RATING */}
            {this.props.storeDetails.rating !== undefined && this.props.storeDetails.rating !== null && this.props.storeDetails.rating > 0
              ? <StarRating
            ...
            {this.props.storeDetails.timings !== null && this.props.storeDetails.timings !== undefined
              ? < TextViewLeftImage
                ...
                // image={Assets.time}
                title={
                  this.props.storeDetails.timings.open != "" && this.props.storeDetails.timings.close != ""
                    ? this.props.storeDetails.timings.open + "-" + this.props.storeDetails.timings.close
  ...
  buttonDirectionsPressed = () => {
    ...
    if (Linking.canOpenURL(strMapsURL)) {
      Linking.openURL(strMapsURL).catch(() => showDialogue('generalNew.canNotLoadDirections'))
    } else {
