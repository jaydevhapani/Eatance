...
export default class ProductsList extends Component {
    ...
    render() {
        ...
        if (this.props.orderDetails.items !== undefined) {
            this.props.orderDetails.items.map(itemToIterate => { numberOfItems += itemToIterate.quantity })
        }
        ...
        return (
            <View style={styles.productsSummaryContainer}>
                <EDRTLText
                    ...
                    title={strings('productDetailsNew.title') + ' (' + numberOfItems + ' ' + (numberOfItems === 1 ? strings('ordersNew.orderedPopUp') : strings('ordersNew.orderedsPopUp')) + ')'} />
...
const styles = StyleSheet.create({
    productsSummaryContainer: { maxHeight: Metrics.screenHeight * 0.75, overflow: 'scroll', borderRadius: 5, marginHorizontal: 20, paddingVertical: 10, backgroundColor: EDColors.white, shadowOpacity: 0.25, shadowRadius: 5, shadowColor: EDColors.text, shadowOffset: { height: 0, width: 0 } },
    ...
    dismissButton: { paddingHorizontal: 20, alignSelf: 'center', marginTop: 10, marginBottom: 0 },
})
