...
export default class EDForgotPassword extends React.Component {
            ...
            <View style={styles.modalContainer} pointerEvents={this.state.isLoading ? 'none' : 'auto'}>
                <View style={styles.modalSubContainer}>
                    <EDRTLView style={{ alignItems: 'center', justifyContent: 'flex-end' }}>
                        <MaterialIcon onPress={this.props.onDismissHandler} size={20} color={EDColors.homeButtonColor} name={'close'} />
                    </EDRTLView>
    ...
    onSubmitButtonHandler = () => {
        ...
        if (this.validationsHelper.validateEmail(this.state.email.trim(), strings('validationsNew.emptyEmail')).length > 0) {
            return;
        }
